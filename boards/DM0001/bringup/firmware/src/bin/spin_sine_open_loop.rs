...
const ALIGN_HOLD_US: u32 = 250_000;
const BOOTSTRAP_PRECHARGE_US: u32 = 8_000;
const BUS_ABORT_DROOP_MV: u32 = 2_000;
const BUS_BACKOFF_DROOP_MV: u32 = 1_000;
const BUS_FEEDBACK_RECOVERY_UPDATES: usize = 256;
const BUS_FEEDBACK_SAMPLE_UPDATES: usize = 16;
const CONTROL_UPDATE_HZ: u32 = 12_000;
...
const OBSERVER_ENABLE_HZ_X100: u32 = 1_000_000;
...
const PROFILE_PAUSE_MS: u32 = 1_000;
const PROFILE_RAMP_UPDATES: usize = 96_000;
const PROFILE_HOLD_UPDATES: usize = 24_000;
const PROFILE_TOTAL_UPDATES: usize = PROFILE_RAMP_UPDATES + PROFILE_HOLD_UPDATES;
const PWM_FREQ_HZ: u32 = 20_000;
const SAMPLE_TIME: SampleTime = SampleTime::CYCLES640_5;
const TELEMETRY_EVERY_UPDATES: usize = 3_000;
...
struct RampProfile {
    name: &'static str,
    launch_amp_pct: u32,
    launch_freq_end_hz_x100: u32,
    launch_updates: usize,
    mid_amp_pct: u32,
    mid_freq_end_hz_x100: u32,
    mid_updates: usize,
    end_amp_pct: u32,
...
async fn main(_spawner: Spawner) {
    ...
    for (profile_index, profile) in RAMP_PROFILES.iter().enumerate() {
        info!(
            "profile_start index={} of={} name={}",
            profile_index + 1,
            RAMP_PROFILES.len(),
            profile.name
        );
        ...
        info!(
            "alignment_start profile={} angle_idx={} amplitude_pct={} hold_us={}",
            profile.name, ALIGN_ANGLE_IDX, ALIGN_AMPLITUDE_PERCENT, ALIGN_HOLD_US
        );
        ...
        info!("alignment_complete profile={}", profile.name);

        let mut phase_acc_q8 = u32::from(ALIGN_ANGLE_IDX) << 8;
        let mut last_angle_idx = ALIGN_ANGLE_IDX;
        let mut last_duties = align_duties;
        let mut adaptive_amplitude_pct = ALIGN_AMPLITUDE_PERCENT.min(profile.launch_amp_pct);
        let mut last_bus_mv = sample_bus_mv(&mut adc1, &mut vbus, &mut vrefint, calibration);
        let bus_backoff_mv = last_bus_mv.saturating_sub(BUS_BACKOFF_DROOP_MV);
        let bus_abort_mv = last_bus_mv.saturating_sub(BUS_ABORT_DROOP_MV);

        info!("state_transition state={}", ControlState::Spinning.as_str());
        info!(
            "sine_profile profile={} total_updates={} ramp_updates={} hold_updates={} launch_updates={} mid_updates={} launch_amp_pct={} mid_amp_pct={} end_amp_pct={} freq_start_hz_x100={} launch_hz_x100_end={} mid_hz_x100_end={} end_hz_x100={} update_hz={} pwm_hz={} bus_backoff_mv={} bus_abort_mv={}",
            profile.name,
            PROFILE_TOTAL_UPDATES,
            PROFILE_RAMP_UPDATES,
            PROFILE_HOLD_UPDATES,
            profile.launch_updates,
            profile.mid_updates,
            profile.launch_amp_pct,
            profile.mid_amp_pct,
            profile.end_amp_pct,
            ELECTRICAL_FREQ_START_HZ_X100,
            profile.launch_freq_end_hz_x100,
            profile.mid_freq_end_hz_x100,
            profile.end_freq_hz_x100,
            CONTROL_UPDATE_HZ,
            PWM_FREQ_HZ,
            bus_backoff_mv,
            bus_abort_mv
        ...
        for update_index in 0..PROFILE_TOTAL_UPDATES {
            let (target_amplitude_pct, electrical_hz_x100) = ramp_targets(*profile, update_index);
            ...
            if update_index % BUS_FEEDBACK_SAMPLE_UPDATES == 0 {
                ...
                if bus_mv < bus_abort_mv {
                    warn!(
                        "bus_abort profile={} idx={} bus_mv={} amp_pct={} elec_hz_x100={}",
                        profile.name,
                        update_index,
                        bus_mv,
                ...
                if bus_mv < bus_backoff_mv {
                    ...
                    if next_amplitude_pct != adaptive_amplitude_pct {
                        ...
                        warn!(
                            "bus_backoff profile={} idx={} bus_mv={} amp_pct={} target_amp_pct={} elec_hz_x100={}",
                            profile.name,
                            update_index,
                            bus_mv,
                            adaptive_amplitude_pct,
            ...
            if electrical_hz_x100 >= OBSERVER_ENABLE_HZ_X100
                && update_index % OBSERVER_FEEDBACK_SAMPLE_UPDATES == 0
            {
            ...
            let phase_step_q8 = clamp_i32(
                i32::try_from(step_q8).unwrap_or(i32::MAX) + phase_correction_q8,
                64,
                8_192,
            ) as u32;
            ...
            let verbose_update = update_index % TELEMETRY_EVERY_UPDATES == 0
                || update_index + 1 == PROFILE_TOTAL_UPDATES;

            if verbose_update {
                info!(
                    "sine_update profile={} idx={} angle_idx={} amp_pct={} target_amp_pct={} elec_hz_x100={} step_q8={} phase_trim_q8={} phase_err={} obs_locked={} bus_mv_sample={} duty_pct=[{},{},{}]",
                    profile.name,
                    update_index,
                    angle_idx,
                    adaptive_amplitude_pct,
        ...
        disable_all_channels(&mut pwm);
        info!(
            "profile_complete index={} of={} name={}",
            profile_index + 1,
            RAMP_PROFILES.len(),
            profile.name
        );
        ...
        if profile_index + 1 != RAMP_PROFILES.len() {
            info!(
                "profile_pause next_index={} pause_ms={}",
                profile_index + 2,
                PROFILE_PAUSE_MS
            );
            hold_disarmed(&mut status_led, PROFILE_PAUSE_MS);
        }
    }

    info!("sine_compare_complete idling_disarmed");
    loop {
...
fn ramp_targets(profile: RampProfile, update_index: usize) -> (u32, u32) {
    if update_index >= PROFILE_RAMP_UPDATES {
        return (profile.end_amp_pct, profile.end_freq_hz_x100);
    }

    let ramp_index = update_index;

    if ramp_index < profile.launch_updates {
        return (
            interpolate(
                ALIGN_AMPLITUDE_PERCENT,
                profile.launch_amp_pct,
    ...
    let mid_updates = profile
        .mid_updates
        .min(PROFILE_RAMP_UPDATES.saturating_sub(profile.launch_updates));
    if ramp_index < profile.launch_updates + mid_updates {
        let mid_index = ramp_index - profile.launch_updates;
        return (
            interpolate(
                profile.launch_amp_pct,
                profile.mid_amp_pct,
    ...
    let high_start = profile.launch_updates + mid_updates;
    let high_index = ramp_index.saturating_sub(high_start);
    let high_updates = PROFILE_RAMP_UPDATES.saturating_sub(high_start).max(1);
...
fn phase_vector_from_angle(angle_idx: u8) -> (i32, i32) {
    ...
    (
        phase_sample_centered(angle_idx),
        clarke_beta(phase_b, phase_c),
    )
}
...
fn update_observer(
    ...
    let mut step_trim_q8 =
        previous.step_trim_q8 * OBSERVER_TRIM_DECAY_NUM / OBSERVER_TRIM_DECAY_DEN;
    let residual_mag_i32 = i32::try_from(residual_mag_mv).unwrap_or(i32::MAX);
    let observer_enabled = electrical_hz_x100 >= OBSERVER_ENABLE_HZ_X100
        && residual_mag_i32 >= OBSERVER_MIN_RESIDUAL_MV;
    ...
    if observer_enabled {
        ...
        phase_error = clamp_i32(
            (cross / i64::from(residual_mag_i32.max(1))) as i32,
            -127,
            127,
        );
...
fn set_sine_duties(
    ...
    let duty_b = compute_phase_duty(
        angle_idx.wrapping_add(85),
        amplitude_counts,
        center_counts,
        max_duty,
    );
    let duty_c = compute_phase_duty(
        angle_idx.wrapping_add(170),
        amplitude_counts,
        center_counts,
        max_duty,
    );
