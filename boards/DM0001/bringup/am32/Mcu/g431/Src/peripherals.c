...
void initCorePeripherals(void)
{
    ...
    MX_TIM3_Init(); /* TIM3 used as INTERVAL_TIMER when TIM2 is input capture */
...
#ifdef USE_TIMER_2_CHANNEL_1
    /* TIM2 used as input capture timer on PA15/TIM2_CH1 */
    LL_GPIO_InitTypeDef GPIO_InitStruct = { 0 };
    LL_AHB2_GRP1_EnableClock(LL_AHB2_GRP1_PERIPH_GPIOA);
...
#ifdef USE_TIMER_2_CHANNEL_1
    /* TIM3 used as interval timer (replaces TIM2) */
    TIM_InitStruct.Prescaler = PCLK_MHZ/2 - 1;
    TIM_InitStruct.CounterMode = LL_TIM_COUNTERMODE_UP;
    TIM_InitStruct.Autoreload = 65535;
    TIM_InitStruct.ClockDivision = LL_TIM_CLOCKDIVISION_DIV1;
...
#else
    /* TIM3 used as input capture timer on PB4/TIM3_CH1 */
    LL_GPIO_InitTypeDef GPIO_InitStruct = { 0 };
...
#ifdef USE_ADC_BEMF
    /* Configure BEMF ADC input pins as analog */
    LL_GPIO_InitTypeDef GPIO_InitStruct = { 0 };
    GPIO_InitStruct.Mode = LL_GPIO_MODE_ANALOG;
    GPIO_InitStruct.Pull = LL_GPIO_PULL_NO;

    GPIO_InitStruct.Pin = BEMF_A_PIN;
    LL_GPIO_Init(BEMF_A_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BEMF_B_PIN;
    LL_GPIO_Init(BEMF_B_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BEMF_C_PIN;
    LL_GPIO_Init(BEMF_C_PORT, &GPIO_InitStruct);
#endif
}
